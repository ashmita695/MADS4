...
def experiment(
    ...
    for path in trajectories:
        ...
        for j in range(n_agents):
            states[j].append(path['observations'][j]) # obseravtions
    ...
    def get_batch(batch_size=256, max_len=K):
        ...
        n_agents = len(trajectories[0]['observations'])
        o, s, a, r, d, rtg, timesteps, mask = [[] for _ in range(n_agents)], [[] for _ in range(n_agents)], [[] for _ in range(n_agents)], [[] for _ in range(n_agents)], [[] for _ in range(n_agents)], [[] for _ in range(n_agents)], [[] for _ in range(n_agents)], [[] for _ in range(n_agents)]
    ...
    if bool(variant['s4_onpolicy']) or bool(variant['s4_offpol_actorcritic']):
        ...
        if variant['online_critictype'] == "resnet":
            ...
            critic2 = FC_critic_resnet(states_dim=state_dim, action_dim=act_dim,
                                      state_enc_size=state_dim, action_enc_size=act_dim,
                                      mutual_enc_size=state_dim + act_dim)
    ...
    if variant['s4_onpolicy_afteroff'] == 0:
        ...
        for iter in range(variant['max_iters']):
            ...
            if current_avg_returns > best_avg_returns:
                ...
                torch.save(current_model.state_dict(), f"smac_corridor_good_best_all_dep_state_emb_act_prev_act_emb_rew_emb_online.pkl")
        ...
        for iter in range(variant['max_iters']):
            outputs, current_model, current_avg_returns = trainer1.train_iteration(num_steps=variant['num_steps_per_iter'], iter_num=iter + 1,
                                               print_logs=True)
            if True:
                logging.info('=' * 80)
                logging.info(f'Iteration {iter + 1}')
            ...
            torch.save({
                ...
                # 'scheduler': scheduler,
            }, f"mujoff_{variant['online_postfix']}_model_iter{iter}_online_after_offline.pkl")
        ...
        for iter in range(variant['max_iters']):
            outputs = trainer2.train_iteration(num_steps=variant['num_steps_per_iter'], iter_num=iter + 1,
                                               print_logs=True)
            if True:
                logging.info('=' * 80)
                logging.info(f'Iteration {iter + 1}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--embed_dim', type=int, default=128) #128-6.841
    ...
    parser.add_argument('--s4_offpol_actorcritic', type=int, default=1)
    ...
    fn = "runlog_" + str(vars(args)['env_name']) + "_" + str(vars(args)['map_name']) + "_" + str(
        vars(args)['dataset']) + f"_{prefix2}_{prefix1}_" + "dependent_all_state_emb_action_prev_act_emb_rew_emb_online" + ".log"
