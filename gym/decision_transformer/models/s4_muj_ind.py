...
class S4_mujoco_wrapper_v3(S4_mujoco_wrapper):
    def __init__(
        ...
        # self.input_projection = nn.Linear(self.input_emb_size * 3 + 6 + 14, self.h)
        # self.input_projection_ind = nn.Linear(self.input_emb_size * 4 + 6, self.h)
        # self.input_projection_ind = nn.Linear(self.input_emb_size * (2) + 2 + self.action_dim, self.h)
        self.input_projection_ind = nn.Linear(self.input_emb_size * (2) + 6 + self.action_dim, self.h)
                                          #                                                                     self.h)
    ...
    def forward(self, obs, states, actions, rewards, rtg, timestep, s4_states=None, running=False, cache=None, goals=None, target_goal=None, **kwargs): # absorbs return_output and transformer src mask
        ...
        if self.batch_mode:
            return self.step_forward(obs, states, actions, rewards, rtg, timestep, s4_states=s4_states, running=running,
                                     **kwargs)
        del_r = 1
        n_agents = len(states)
        ...
        #     final_states = target_goal
        #     #final_states = states[:,-1,:2].unsqueeze(1).expand((states.shape[0], states.shape[1], 2))
        #     states = torch.cat([states, final_states], dim=-1)
        # # print(self.config.recurrent_mode) #False
        # if self.config.recurrent_mode:
        #     self.pre_val_setup()
        #     s4_states = [r.detach() for r in self.get_initial_state((batch_size), states.device)]
        #     ret_y = torch.zeros_like(actions[:,:sequence_len-del_r,...])
        #     for i in range(ret_y.shape[1]):
        #         given_action = actions[:,i,...]
        #         curr_ret, s4_states = self.step_forward(states[:,i+1,...].unsqueeze(1), given_action.unsqueeze(1),
        #                                                 rewards[:,i,...].unsqueeze(1), rtg[:,i,...].unsqueeze(1),
        #                                                 timestep[:,i+1,...].unsqueeze(1), s4_states=s4_states)
        #         ret_y[:,i,:] = curr_ret
        #     return None, ret_y, None
        nan_input_state = False
        for i in range(n_agents):
            ...
            global_state_embed = self.state_encoder(
                states[i][:, del_r:, ...].reshape(-1, self.state_dim).type(torch.float32).contiguous())
            ...
            #         #     [concatenated_actions, torch.zeros_like(concatenated_actions[..., :pad_size])], dim=-1)
            #
            ...
            # u = torch.cat([state_embed, reward_embed, action_embed, prev_action_embed], dim=-1)
            u = torch.cat(
                [one_hot_agent_id[:, del_r:, ...].reshape(-1, n_agents), reward_embed, obs_embed, action_embed], dim=-1)
            ...
            if torch.isnan(ret_temp).any() and not running:
                print('nan during training!!')
