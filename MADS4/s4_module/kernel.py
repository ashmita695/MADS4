...
try:
    import pykeops
    from cauchy import cauchy_conj
    has_pykeops = True
    log.info("Pykeops installation found.")
except:
    has_pykeops = False
    from cauchy import cauchy_slow
...
class SSKernelNPLR(OptimModule):
    ...
    def setup_step(self, mode='dense'):
        ...
        #print(f"LOGXX Lshape {self.L.shape}")
        #print(f"LOGXX Ldtype {self.L.dtype}")
        #print(f"LOGXX dAshape {self.dA.shape}")
        #print(f"LOGXX dAtype {self.dA.dtype}")
    ...
    def __init__(
        ...
        **kernel_args,
        ...
        if mode == "real":  # For testing and ablation purposes
            ...
            if measure == "random":
                w, P, B, C, _ = hippo.random_dplr(self.N, rank=rank, H=n_ssm, dtype=dtype, scaling = 'random',
                                                  random_real=True, random_imag=True, **measure_args)
            elif measure == 'hippo':
