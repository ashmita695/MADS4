...
from decision_transformer.evaluation.evaluate_episodes_warehouse_ind import evaluate_episode, evaluate_episode_rtg
...
from decision_transformer.envs import REGISTRY as env_REGISTRY
...
def experiment(
    ...
    if env_name == 'StarCraft2':
        ...
        args = {
            ...
            "n_agents": 6,  # Specify the number of agents
            "episode_limit": 500,
            "difficulty": "easy",
            "seed": 42,
            "render": False,
        ...
        env = env_REGISTRY[env_name](**args)
        env.real_env = env
        online_train_env = env
        global_obs_dim = get_dim_from_space(online_train_env.real_env.share_observation_space)
        local_obs_dim = get_dim_from_space(online_train_env.real_env.observation_space)
        action_dim = get_dim_from_space(online_train_env.real_env.action_space)
        num_agents = online_train_env.num_agents
    ...
    dataset_path = f'data_ma/warehouse/rware_small/rware_small_1000/expert/rware_small_6ag_easy_v4_expert.pkl'
    ...
    print(f"offline data: global obs dim:{state_dim}; local obs dim:{obs_dim}; action dim:{act_dim}; num agents:{n_agents}")
    ...
    def get_batch(batch_size=256, max_len=K):
        ...
        for i in range(batch_size):
            ...
            for j in range(n_agents):
                ...
                acts_ = traj['actions'][j][si:si + max_len]
                actions_one_hot = np.array([one_hot(int(acts_[k][0]), act_dim) for k in range(acts_.shape[0])])
                a[j].append(actions_one_hot.reshape(1, -1, act_dim))
                # a[j].append(traj['actions'][j][si:si + max_len].reshape(1, -1, act_dim))
    ...
    if variant['s4_onpolicy_afteroff'] == 0:
        ...
        for iter in range(variant['max_iters']):
            ...
            if current_avg_returns > best_avg_returns:
                ...
                torch.save(current_model.state_dict(), f"rware_small_6ag_expert_independent_all_agent_id_rew_emb_global_emb_local_emb_prev_emb_act_emb_emb_128_K_500.pkl")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--env_name', type=str, default='warehouse')
    parser.add_argument('--map_name', type=str, default='rware_small_6ag')
    parser.add_argument('--dataset', type=str, default='expert')  # good, medium, poor
    parser.add_argument('--mode', type=str, default='normal')  # normal for standard setting, delayed for sparse
    parser.add_argument('--K', type=int, default=500)  # max sequence length for training
    parser.add_argument('--pct_traj', type=float, default=1.)
    parser.add_argument('--seed', type=int, default=123)
    parser.add_argument('--batch_size', type=int, default=64)
    parser.add_argument('--model_type', type=str, default='s4v3')
    parser.add_argument('--embed_dim', type=int, default=128) #128
    ...
    fn = "runlog_" + str(vars(args)['env_name']) + "_" + str(vars(args)['map_name']) + "_" + str(
        vars(args)['dataset']) + f"_{prefix2}_{prefix1}_" + "independent_all_agent_id_rew_emb_global_emb_local_emb_prev_emb_act_emb_emb_128_K_500" + ".log"
