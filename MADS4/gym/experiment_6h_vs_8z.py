...
def experiment(
    ...
    # model.load_state_dict(torch.load(f"smac_best_all_dep_no_tanh_no_act_emb_ablated.pkl"))
    # model.load_state_dict(torch.load(f"muj_{variant['online_postfix']}_model_iter0.pkl"))
    # model.load_state_dict(torch.load(f"mujfinal_latest_model_end.pkl"))
    #
    ...
    if variant['s4_onpolicy_afteroff'] == 0:
        ...
        for iter in range(variant['max_iters']):
            ...
            if current_avg_returns > best_avg_returns:
                ...
                torch.save(current_model.state_dict(), f"6h_8z_good_agent_id_obs_embed_state_emb_prev_act_embed_act_rew_embed_full_offline.pkl")
...
if __name__ == '__main__':
    ...
    fn = "runlog_" + str(vars(args)['env_name']) + "_" + str(vars(args)['map_name']) + "_" + str(
        vars(args)['dataset']) + f"_{prefix2}_{prefix1}_" + "agent_id_obs_embed_state_emb_prev_act_embed_act_rew_embed_full_offline" + ".log"
