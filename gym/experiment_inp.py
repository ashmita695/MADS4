...
from decision_transformer.evaluation.evaluate_episodes_ind import evaluate_episode, evaluate_episode_rtg
from decision_transformer.models.decision_transformer import DecisionTransformer
from decision_transformer.models.s4_muj_ind import *
from decision_transformer.models.mlp_bc import MLPBCModel
from decision_transformer.training.act_trainer import ActTrainer
from decision_transformer.training.seq_trainer_ind import SequenceTrainer
...
def experiment(
    ...
    env_name, dataset = variant['env_name'], variant['dataset']
    ...
    #     if bool(variant['overhead_rtg_test']):
    #         env_targets = [3600, 2600, 1800]
    ...
    #     max_ep_len = 1000
    #     env_targets = [1, 0.5, 1.1]
    #     scale = 0.33
    # else:
    #     raise NotImplementedError
    # if bool(variant['overhead_rtg_test']):
    #     env_targets.append(int(1.05*env_targets[0]))
    #     env_targets.append(int(0.95 * env_targets[0]))
    #     env_targets.append(int(1.05 * env_targets[1]))
    #     env_targets.append(int(0.95 * env_targets[1]))
    #
    # if model_type == 'bc':
    ...
    for path in trajectories:
        ...
        # act_dim = path['actions'][0].shape[1]
        act_dim = action_dim
        # print(f"obs dim:{obs_dim}; state dim:{state_dim}; action dim:{act_dim}")
    ...
    # print(f"offline state shape:{len(states[0])}")
    # print(f"offline trajectory length:{len(traj_lens[0])}")
    ...
    # print(f'state_mean for 1st agent: {state_mean[0]}, state_std for 1st agent: {state_std[0]}')
    print('=' * 50)

    scale = 1.0
    env_targets = [np.max(returns[0])*1.10, np.min(returns[0])*0.90] # 10% increase and decrease
    ...
    # p_sample = 1/num_trajectories # uniform


    # s4_ant_bias = variant['s4_ant_train_bias']
    # if s4_ant_bias > 0:
    #     # p_sample1 = None
    #     p_sample1 = p_sample
    #     # p_sample1 = traj_lens[sorted_inds] / sum(traj_lens[sorted_inds])
    #     p_sample2 = returns[sorted_inds] / sum(returns[sorted_inds])
    #     p_sample2 = returns[sorted_inds] / sum(returns[sorted_inds])
    ...
    if variant['s4_onpolicy_afteroff'] == 0:
        ...
        # model.load_state_dict(torch.load(f"muj_{variant['online_postfix']}_model_iter0.pkl"))
        ...
        for iter in range(variant['max_iters']):
            ...
            if current_avg_returns > best_avg_returns:
                ...
                torch.save(current_model.state_dict(),
                           f"Independent_6h_8z_good_agent_id_rew_embed_global_state_emb_obs_embed_prev_act_embed_act_offline_nplr_emb_96_iter_1000_good.pkl")

            print(f"best average returns seen so far: {best_avg_returns}")

            logging.info(
                f'Iteration {iter}: return = {current_avg_returns}: best return so far = {best_avg_returns}')

            model.to(torch.float32)
            torch.save(model.state_dict(), f"muj_{variant['online_postfix']}_model_iter{iter}.pkl")
            ...
            #     # 'scheduler': scheduler,
            # }, f"muj_{variant['online_postfix']}_model_iter{iter}.pkl")
            # torch.save(model.state_dict(), f"muj_{variant['online_postfix']}_model_oldzipiter{iter}.pkl", _use_new_zipfile_serialization = False)
            # torch.save(model.state_dict(), f"muj_{variant['online_postfix']}_model_prot2iter{iter}.pkl", pickle_protocol=pickle.HIGHEST_PROTOCOL)
    # else:
    ...
    #     for iter in range(variant['s4_onpolicy_afteroff']):
    #         outputs = trainer1.train_iteration(num_steps=variant['num_steps_per_iter'], iter_num=iter+1, print_logs=True)
    #         if True:
    #             logging.info('=' * 80)
    #             logging.info(f'Iteration {iter+1}')
    #             for k, v in outputs.items():
    ...
    #         if log_to_wandb:
    #             wandb.log(outputs)
    #         #torch.save(model.state_dict(), f"mujoff_{variant['online_postfix']}_model_iter{iter}.pkl")
    #         torch.save({
    #             'model': model,
    #             'model_state_dict': model.state_dict(),
    #             'optimizer': optimizer,
    #             'optimizer_state_dict': optimizer.state_dict()
    #             #'scheduler': scheduler,
    #         }, f"mujoff_{variant['online_postfix']}_model_iter{iter}.pkl")
    #         if env_name=='walker2d':
    #             topass = 3300
    #         elif env_name=='halfcheetah':
    #             if variant['dataset'] == 'medium-replay':
    #                 topass = 4000
    #             else:
    #                 topass = 4900
    #         else:
    ...
    #     for iter in range(variant['max_iters']):
    #         outputs = trainer2.train_iteration(num_steps=variant['num_steps_per_iter'], iter_num=iter+1, print_logs=True)
    #         if True:
    #             logging.info('=' * 80)
    #             logging.info(f'Iteration {iter+1}')
    #             for k, v in outputs.items():
    ...
    #         if log_to_wandb:
    #             wandb.log(outputs)
    #         #torch.save(model.state_dict(), f"mujon_{variant['online_postfix']}_model_iter{iter}.pkl")
    #         torch.save({
    #             'model': model,
    #             'model_state_dict': model.state_dict(),
    #             'optimizer': optimizer,
    #             'optimizer_state_dict': optimizer.state_dict()
    #             #'scheduler': scheduler,
    #         }, f"mujon_{variant['online_postfix']}_model_iter{iter}.pkl")
    model.to(torch.float32)
    ...
    #     'optimizer_state_dict': optimizer.state_dict(),
    #     # 'scheduler': scheduler,
    # }, f"mujfinal_{variant['online_postfix']}_model_iter{iter}.pkl")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--K', type=int, default=50)  # max sequence length for training
    parser.add_argument('--pct_traj', type=float, default=1.)
    parser.add_argument('--seed', type=int, default=123)
    parser.add_argument('--batch_size', type=int, default=64)
    parser.add_argument('--model_type', type=str, default='s4v3')
    parser.add_argument('--embed_dim', type=int, default=96) #128
    ...
    # S4 related
    parser.add_argument('--s4weight_decay', type=float, default=0.0)
    parser.add_argument('--s4internal', type=int, default=96)#hidden state dim 96
    ...
    parser.add_argument('--s4dropoutval', type=float, default=0.0)
    parser.add_argument('--s4_layers', type=int, default=3)
    ...
    parser.add_argument('--online_postfix', type=str, default='latest_independent')
    ...
    fn = "runlog_" + str(vars(args)['env_name']) + "_" + str(vars(args)['map_name']) + "_" + str(
        vars(args)['dataset']) + f"_{prefix2}_{prefix1}_"  + "independent" + "agent_id_rew_embed_global_state_emb_obs_embed_prev_act_embed_act_offline_nplr_emb_96_iter_1000_good" + ".log"
